...
# =============================================================================
# NML-1115: the book INDEX — the faction key without the network
# =============================================================================
## `faction_folder` is the second half of every registry key (`RulesRegistry.faction_of_unit`,
## and `p.faction_folder` on the Rust side), and it used to be known only once the army book
## had been fetched. A failed fetch left it "" and every faction-scoped rule lookup silently
## collapsed to the `common` fallback — the NML-1114 root cause. `army_books_index.json` is
## PUBLIC and text-free (armyId -> book name + slug + version, generated by
## `tools/army_books_index.py`; no rule text ever enters this repo, THIRD_PARTY.md), so the
## key is set in the NETWORK-FREE half of the import and a fetch failure can now cost only
## the rule TEXT, never the key.
const ARMY_BOOKS_INDEX_PATH := "res://assets/solo/army_books_index.json"
static var _books_index: Variant = null


## The index entry for (armyId, game-system abbrev), or {} when the book is not indexed
## (a hand-imported list from a book outside the bundled pools). Reads the file once.
static func book_index_entry(army_id: String, system_abbrev: String) -> Dictionary:
	if _books_index == null:
		var parsed: Variant = JSON.parse_string(FileAccess.get_file_as_string(ARMY_BOOKS_INDEX_PATH))
		_books_index = parsed if parsed is Dictionary else {}
	var section: Variant = (_books_index as Dictionary).get(system_abbrev, {})
...
## names guessed out of upgrade LABEL text (NML-1105). NML-1115: the faction NAME
## and folder are answered here too, from the offline book index — the live book
## may refine them, but never has to be reached for the registry key. Only
## rule_descriptions and spells still need the network.
...
## Stamps `faction_name` / `faction_folder` from the offline book index. No-op when the
## book is not indexed, so an unindexed import keeps today's behaviour (API, then the
## list-name folder guess).
static func _apply_book_index(army: OPRArmy) -> void:
	var entry := book_index_entry(army.army_id, army.game_system_abbrev)
	if entry.is_empty():
		return
	army.faction_name = str(entry.get("book_name", ""))
	army.faction_folder = str(entry.get("faction_folder", ""))
...
func _parse_tts_weapon(data) -> OPRWeapon:
	...
	# Army Forge data carries stray leading/trailing spaces (" Heavy Sniper Rifle" produced
	# "fires  Heavy Sniper Rifle" with a double space in the battle log) — strip at the source.
	weapon.name = str(data.get("name", data.get("label", "Unknown"))).strip_edges()
...
func _parse_army_forge_json(json_text: String, source_name: String = "") -> OPRArmy:
	...
	# NML-1115: the registry key from the offline index, before the network is touched.
...
func _parse_weapon(data) -> OPRWeapon:
	...
	var weapon = OPRWeapon.new()
	weapon.name = str(data.get("name", data.get("label", "Unknown Weapon"))).strip_edges()   # source-strip (see above)
