...
## Cap on ABSOLUTE anti-stacking relaxation passes (resolve_overlaps). Larger than
## MAX_ITERATIONS because that pass must clear a base against EVERY other base (same unit
## included) and can need several relaxation steps to walk a wedged base out through a
## gap; still a hard bound before the directional escape fallback takes over.
const MAX_OVERLAP_ITERATIONS := 24

## Directions sampled by the escape scan (the fallback that guarantees a fully-surrounded
## drop still ends overlap-free): more = a tighter, shorter escape.
...
## ABSOLUTE anti-stacking: two model bases may NEVER overlap (edge_distance < 0) — for ANY
## pair (same unit, own other unit, enemy). This is the hard physical no-stack rule,
## distinct from the 1" unit-spacing rule (which binds only BETWEEN units and has a charge
## exception). Given the item's freshly-built BaseShapes (mutated in place) and a flat
## Array of obstacle BaseShapes (EVERY other alive base, unit-agnostic), relaxes the item
## out until it clears every obstacle by >= 0 gap, then returns the total world-XZ
## translation (metres). Base CONTACT (gap ~0) is left untouched — only true overlap is
## pushed. Uses a resultant-vector relaxation (naturally walks toward the least-crowded
## side). If a pathological FULLY-SURROUNDED drop still overlaps after the relaxation cap
## (e.g. a symmetric wedge the resultant cancels in), a directional escape scan moves the
## item to the NEAREST fully-clear position instead — a finite obstacle set always has one,
## so the item is never left stacked.
static func resolve_overlaps(item_shapes: Array, obstacle_shapes: Array) -> Vector2:
	...
	for _iter in range(MAX_OVERLAP_ITERATIONS):
		...
		if resultant.length() < RESOLVE_EPSILON_INCHES:
			# Symmetric wedge: the resultant cancels while overlap remains. Jump straight to
			# the escape scan rather than spend iterations oscillating in place.
			break
		step = resultant * INCHES_TO_METERS
...
## Directional escape (fallback): moves item_shapes (mutated) to the NEAREST position that
## clears every obstacle, and returns that translation. Uses conservative bounding circles
## (a base is contained by its bounding circle, so bounding-circle clearance implies base
## clearance), scanning ESCAPE_SCAN_DIRECTIONS rays for the one needing the least travel.
## No-op when already clear. Guarantees termination with zero overlap for any finite set.
...
## Smallest distance (metres) to slide item_shapes along unit direction `u` so every
## item-base bounding circle clears every obstacle bounding circle. Solves, per pair, the
## quadratic |e + t*u| >= Rsum for its upper root, and takes the max across all pairs.
