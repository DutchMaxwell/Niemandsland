...
static func solve(p: Dictionary) -> Dictionary:
	...
	# Quick Shot (army-book): "may shoot after using Rush actions" — every reachable firing spot is a
	# this-turn shot, not only those inside the Advance band.
	var quick_shot: bool = bool(p.get("quick_shot", false))
	...
	if is_shooter and not shooters.is_empty():
		...
		if not kept.is_empty():
			...
			# A Quick Shot unit shoots after Rushing too, so its WHOLE kept pool is this-turn shots.
			var now_shots: Array = kept if quick_shot \
				else kept.filter(func(c): return float((c as Dictionary)["travel_in"]) <= advance_m * in_per_m + 0.01)
	...
	# Quick Shot (army-book): the unit "may shoot after using Rush actions" — a shooting spot beyond
	# the Advance band keeps its volley instead of being demoted to a dry Rush.
	var shoot := did_shoot and (action == "advance" or bool(p.get("quick_shot", false)))
	return {
		"used": true,
		"goal": chosen["pos"],
...
## (b) our chosen shot is covered while the current spot's shot is exposed (a cover upgrade);
## (c) an objective-bound shooter reaches the seize ring while keeping a shot (anchor generalisation); or
## (d) the chosen spot's volley is MATERIALLY richer than the current spot's (NML-007 "solver too quiet":
##     both absolute + relative margins must clear, so dice-noise-sized gains never break move-commitment).
...
const EV_UPGRADE_ABS := 0.75   # (d) minimum absolute EV gain, expected wounds
const EV_UPGRADE_REL := 1.25   # (d) minimum relative EV gain (chosen ≥ 1.25 × current)
static func _worth_overriding(chosen: Dictionary, from_eval: Dictionary, did_shoot: bool,
	...
	if float(chosen["ev"]) >= float(from_eval["ev"]) + EV_UPGRADE_ABS \
			and float(chosen["ev"]) >= float(from_eval["ev"]) * EV_UPGRADE_REL:
		return true   # (d) a materially richer volley from another reachable spot
	return false
