...
## The numeric rating of a rule string ("Retaliate(3)" -> 3; `fallback` when unrated).
static func rule_rating(rule: String, fallback: int = 0) -> int:
	var s := rule.strip_edges()
	var open := s.find("(")
	if open < 0:
		return fallback
	var inner := s.substr(open + 1).trim_suffix(")").strip_edges()
	return int(inner) if inner.is_valid_int() else fallback


## Coverage wave (2026-07-23): the GENERIC ALIAS LAYER. Every effective rule of `unit` whose
## registry entry resolves to `primitive`, with its params and parsed rating — so family resolvers
## consume DATA aliases (Plaguebound → Regeneration ignore_target 6) instead of hardcoded names.
## Effective rules = direct special_rules + item-granted (item_grants); each name counts once.
## Returns [{name, params, rating}]. Empty when the unit or map is absent (callers keep their
## name-based fallbacks, so missing data never breaks wave-1..6 behaviour).
static func unit_rules_of_primitive(unit: GameUnit, primitive: String) -> Array:
	var out: Array = []
	if unit == null or primitive.is_empty():
		return out
	var system := system_of_unit(unit)
	var faction := faction_of_unit(unit)
	var seen := {}
	var raws: Array = []
	for r in unit.get_special_rules():
		raws.append(str(r) if r is String else str((r as Dictionary).get("name", "")))
	for granted_list in (unit.unit_properties.get("item_grants", {}) as Dictionary).values():
		for granted in granted_list:
			raws.append(str(granted))
	for raw in raws:
		var n := base_rule_name(str(raw))
		if n.is_empty() or seen.has(n):
			continue
		seen[n] = true
		var entry := lookup(system, faction, n)
		var p: Variant = entry.get("primitive")
		if p is String and str(p) == primitive:
			out.append({"name": n, "params": entry.get("params", {}), "rating": rule_rating(str(raw))})
	return out


## The best (largest numeric) value of one param across a unit's rules of `primitive` — the common
## "any rule of this family grants it" reader. `fallback` when none carries the family.
static func best_primitive_param(unit: GameUnit, primitive: String, key: String, fallback: Variant) -> Variant:
	var best: Variant = null
	for e in unit_rules_of_primitive(unit, primitive):
		var v: Variant = ((e as Dictionary).get("params", {}) as Dictionary).get(key)
		if v == null:
			continue
