...
func _execute_move(unit: GameUnit, goal: Vector3, inches: float, allow_contact: bool,
	...
	# EPOCH_27_TERRAIN_DEBUFF (sweep A, rows Dangerous/Difficult Terrain Debuff): a rule the
	...
	var debuffs_on: bool = AiActRecorder.rules_epoch >= AiActRecorder.EPOCH_27_TERRAIN_DEBUFF
	...
	if not ignores_difficult and (_trails_cross_difficult(trails, trail_radii_m) or difficult_debuff):
	...
	if not allow_contact and (avoid or avoid_dangerous) \
		...
		var r2 := reach
		if not ignores_difficult and (_trails_cross_difficult(t2, trail_radii_m) or difficult_debuff):
	...
	if not flying:
		...
		for i in range(models.size()):
			# EPOCH_27_TERRAIN_DEBUFF: the carried rule is a hazard the cells never are —
...
## EPOCH_27_TERRAIN_DEBUFF (sweep A) — the granted-terrain-debuff read over the JOINED chain
...
func _chain_has_special_rule(unit: GameUnit, rule: String) -> bool:
	if unit == null:
		return false
	if unit.has_special_rule(rule):
		return true
	for h in unit.get_attached_heroes():
		if h is GameUnit and (h as GameUnit).has_special_rule(rule):
			return true
	return false
...
func _charge_path_probe_impl(unit: GameUnit, target: GameUnit, band_in: float) -> Dictionary:
	...
	# EPOCH_27_TERRAIN_DEBUFF: the charge runs the same p.11 cap a carried Difficult
	...
	if not ignores_difficult and (_trails_cross_difficult(trails, radii_m)
			or (AiActRecorder.rules_epoch >= AiActRecorder.EPOCH_27_TERRAIN_DEBUFF
