...
> **Game data note:** One Page Rules (OPR) unit stats / army lists **and rule
> descriptions** are **not** bundled or redistributed. They are loaded **only at
> runtime via the Army Forge API**, and never enter this repository or a shipped
> build.
...
| OPR rule descriptions | OPR content | **no — API only** | never bundled and never shipped; fetched from Army Forge at runtime. Internal corpus runs may read a private, non-distributed snapshot instead (`NML_ARMY_BOOKS_DIR`) so a recording can name the exact texts it used |
| Army-book index | ids + titles only | yes | `assets/solo/army_books_index.json` — army-book id → faction slug, book title, version; **no rule text, no stats** (generated by `tools/army_books_index.py`, so the registry key never depends on the network) |
