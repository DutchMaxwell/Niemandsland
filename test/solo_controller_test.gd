...
# === Stage 1 record honesty: the lookahead rule string must name what moves its number ===
# The record advertised only "highest immediate payoff", while the score multiplies an un-activated
# target by 1.4 and doubles a marker's worth in the final round. Text-only fix — both numbers are
# pinned here, so the text can never be made true by changing the decision.

func _arm_bow(u: GameUnit) -> void:
...
func _payoff_solo(units: Array) -> SoloController:
	var army: OPRArmyManager = auto_free(OPRArmyManager.new())
	for u in units:
...
func test_lookahead_record_names_the_two_factors_that_move_its_payoff() -> void:
	# 20" out: inside the shooting reach (24" + 6" Advance), outside the 12" rush band — only the
	# shooting term scores, so the denial weight is the ONLY difference between the two readings.
	var actor := _unit(2, [Vector3.ZERO])
	_arm_bow(actor)
	var enemy := _unit(1, [Vector3(20.0 * INCH_M, 0, 0), Vector3(20.0 * INCH_M, 0, 0.05),
		Vector3(20.0 * INCH_M, 0, 0.10), Vector3(20.0 * INCH_M, 0, 0.15)])
	var solo := _payoff_solo([actor, enemy])
	# BEHAVIOUR PIN (unchanged): a target that has not activated yet is still worth exactly x1.4.
	var fresh := solo.activation_payoff(actor)
	enemy.is_activated = true
	var spent := solo.activation_payoff(actor)
	assert_float(spent).is_greater(0.0)
	assert_float(fresh).is_equal_approx(spent * 1.4, 0.0001)

	assert_object(solo._select_unit_lookahead([actor])).is_equal(actor)

	assert_int(solo.decision_log.size()).is_equal(1)
	var rec: Dictionary = solo.decision_log[0]
	assert_str(str(rec.get("kind", ""))).is_equal("lookahead")
	# THE FIX: both hidden factors are named where the payoff is claimed.
	var rule := str(rec.get("rule", "")).to_lower()
	assert_str(rule).override_failure_message(
		"the un-activated multiplier is invisible in the rule text: %s" % rule).contains("1.4")
	assert_str(rule).contains("not activated")
	assert_str(rule).override_failure_message(
		"the final-round marker doubling is invisible in the rule text: %s" % rule).contains("final round")
	assert_str(rule).contains("doubl")


func test_lookahead_payoff_still_doubles_a_marker_in_the_final_round() -> void:
	var actor := _unit(2, [Vector3.ZERO])
	var solo := _payoff_solo([actor])   # no enemy: the marker term is the whole score
	solo.objectives_provider = func() -> Array: return [Vector3(0.0, 0, 0.25)]   # ~9.8", inside Rush
	solo.objective_owner_of = func(_i: int) -> int: return 0
	assert_float(solo.activation_payoff(actor)).is_equal_approx(2.5, 0.0001)
	solo.game_rounds = 4
	solo.round_provider = func() -> int: return 4
	assert_float(solo.activation_payoff(actor)).is_equal_approx(5.0, 0.0001)
