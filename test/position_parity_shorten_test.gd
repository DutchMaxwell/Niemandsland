...
func test_pinned_straggler_repair_feeds_whole_unit_shorten() -> void:
	var all_pins: Dictionary = JSON.parse_string(FileAccess.get_file_as_string(
		"res://test/fixtures/position_parity/whole_unit_shorten.json"))
	var pin: Dictionary = all_pins["repair_probe"]
	var solo: SoloController = auto_free(SoloController.new())
	add_child(solo)
	var models: Array = []
	var cfg: Array = []
	for p in pin["planned_world"]:
		var unit := GameUnit.new()
		unit.unit_properties = {"base_size_round":int(round(float(pin["radius_m"]) * 2000.0))}
		var model := ModelInstance.new()
		model.unit = unit
		model.properties = {"tough":1}
		var node: Node3D = auto_free(Node3D.new())
		...
		model.node = node
		models.append(model)
		cfg.append(Vector3(p[0],p[1],p[2]))
	solo._pull_stragglers_coherent_world(models,cfg,[],9.0)
	for i in cfg.size():
		var p: Array = pin["expected_world"][i]
		assert_float((cfg[i] as Vector3).distance_to(Vector3(p[0],p[1],p[2])) / 0.0254).is_less_equal(
			float(pin["tolerance_in"]))
	for model in models:
		model.unit = null
		model.node = null
