...
func test_urgency_respects_low_mission_focus_knobs() -> void:
	# NML-211: low-focus MACHINERY stays covered — the urgency is a full-focus sharpening and must
	# not fire for a low-focus knob set (the later weak-persona rebuild relies on this gate).
	var pair := _urgency_board()
	var sc: SoloController = pair[0]
	sc.game_rounds = 4
	sc.round_provider = func() -> int: return 4
	sc.difficulty_seed = 7
	var loose := SoloDifficulty.new()
	loose.grade_name = "loose-tier"
	loose.ev_noise = 0.4
	loose.mission_focus = 0.35
	loose.persistence = 0.0
	loose.base_seed = 7
	sc.set_difficulty(2, loose)
	sc.activate_next_ai_unit()
	var kinds: Array = []
	for rec in sc.drain_decisions():
		kinds.append(str((rec as Dictionary).get("kind", "")))
