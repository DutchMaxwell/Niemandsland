...
func test_footprint_cap_limits_wide_models() -> void:
	...
	var base_long_mm := 40
	var fit = mgr._compute_model_fit(aabb, base_long_mm, 0, false)
...
func test_flying_adds_lift() -> void:
	...
	var base_long_mm := 40
	var grounded = mgr._compute_model_fit(aabb, base_long_mm, 0, false)
	var flying = mgr._compute_model_fit(aabb, base_long_mm, 0, true)
...
func test_degenerate_aabb_returns_fallback() -> void:
	var mgr := _mgr()
	var fit = mgr._compute_model_fit(AABB(), 32, 0, false)
