extends SceneTree
## Reproducible ground review on the bundled tutorial board. Run on a real display:
## godot --path . -s res://test/manual/grassland_capture.gd -- <output_directory>
## Use isolated XDG_DATA_HOME / XDG_CONFIG_HOME directories with populated asset caches.
## Animated atmosphere is hidden in both revisions to isolate the surface comparison.
