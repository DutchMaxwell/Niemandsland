...
func test_full_and_basic_grades_differ_on_a_rush_reachable_marker() -> void:
	# Marker 13" away: seizable within a Rush (12+3) but NOT an Advance (6+3). FULL (kriegsherr) still holds
	# the guaranteed shot; BASIC (veteran) gives it up to close on the rush-reachable marker — weaker discipline.
	var kh := _shooter_board(13.0)
	var sc_kh: SoloController = kh[0]
	sc_kh.difficulty_seed = 7
	sc_kh.set_difficulty(2, SoloDifficulty.for_grade("kriegsherr", 7))
	sc_kh.activate_next_ai_unit()
	assert_int(int(sc_kh.last_report.get("action", -1))).is_equal(AiDecision.Action.HOLD)

	var vt := _shooter_board(13.0)
	var sc_vt: SoloController = vt[0]
	sc_vt.difficulty_seed = 7
	sc_vt.set_difficulty(2, SoloDifficulty.for_grade("veteran", 7))
	sc_vt.activate_next_ai_unit()
	assert_int(int(sc_vt.last_report.get("action", -1))).is_not_equal(AiDecision.Action.HOLD)


func test_rekrut_never_holds_fire() -> void:
...
func test_ranged_order_aborts_with_a_reason_when_no_target_in_range() -> void:
	# Enemy 40" away (out of a 24" rifle) with an uncontrolled marker: the shooter has NO clean shot, so the
	# standing hold-fire order ABORTS ("reposition") and the tree's marker move stands — never a frozen unit.
	var gun := _unit(2, [Vector3.ZERO], [], "Gunners")
	_arm(gun, [{"name": "Rifle", "range": 24, "attacks": 3, "rules": []}])
	...
	var hf := _hold_fire_record(sc.drain_decisions())
	assert_bool(hf.is_empty()).is_false()
	assert_str(str((hf.get("data", {}) as Dictionary).get("continuity", ""))).is_equal("abort")
...
func test_persistence_tier_separates_the_grades() -> void:
	assert_int(SoloDifficulty.for_grade("rekrut").persistence_tier()).is_equal(0)
	assert_int(SoloDifficulty.for_grade("veteran").persistence_tier()).is_equal(1)
	assert_int(SoloDifficulty.for_grade("kriegsherr").persistence_tier()).is_equal(2)
	assert_int(SoloDifficulty.for_grade("albtraum").persistence_tier()).is_equal(2)
	assert_float(SoloDifficulty.for_grade("kriegsherr").to_dict().get("persistence", -1.0)).is_equal(1.0)
