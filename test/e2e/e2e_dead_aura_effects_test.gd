...
func test_piercing_fighter_aura_reaches_ev_and_named_dice_parts() -> void:
	for pair in [["gf", "orc_marauders"], ["gf", "wormhole_daemons_of_war"],
			["gff", "orc_marauders"], ["gff", "wormhole_daemons_of_war"],
			["aof", "rift_daemons_of_war"], ["aofr", "rift_daemons_of_war"],
			["aofs", "rift_daemons_of_war"], ["aof", "goblins"]]:
		var system := str(pair[0])
		var faction := str(pair[1])
		var u := _unit(["Piercing Fighter Aura"], system, faction)
		var grants := AiEv.aura_granted_rules([u])
		assert_array(grants).is_equal(["Piercing Fighter"])
		...
		for reach in [0, 24]:
			...
			for charging in [false, true]:
				...
				assert_array(parts).is_equal([{"name": "Piercing Fighter", "bonus": 1}] if reach == 0 else [])
			profile["rules"] = ["Piercing Fighter"]
			var deduped: Array = AiEv.stamp_conditional_ap([profile], u)
			assert_int(deduped[0].get("cond_ap", []).size()).is_equal(1)


func test_piercing_fighter_save_step_logs_one_named_ap_bonus() -> void:
	_main.solo_ai_slots = {2: true}
	_main._ensure_solo_controller()
	_main.opr_army_manager.game_phase = OPRArmyManager.GamePhase.PLAYING
	_main._solo_batch = true
	var u := _carrier("Piercing Fighter", "gf", "orc_marauders")
	var foe := _unit([], "gf", "orc_marauders")
	await _main._solo_resolve_saves(u, foe, "Blade", [4], 1, 4,
		{"range": 0, "ap": 0, "rules": ["Piercing Fighter"]}, false, true)
	var named := 0
	var text := ""
	for entry in _main.battle_log.entries():
		var line := str(entry["text"])
		text += line + "\n"
		if line.contains("Piercing Fighter: AP(+1)"):
			named += 1
