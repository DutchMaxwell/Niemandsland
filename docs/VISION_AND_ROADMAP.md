...
- `/home/user/openTTS/relay/HOST_RECONNECT.md` — host-reconnect spec (0.4)
- `/home/user/openTTS/relay/test_relay_server.py` — 30 pytest cases gating relay changes
- `/home/user/openTTS/scripts/relay_multiplayer_peer.gd` — WebSocket transport, ENet-free (browser-ready)
- `/home/user/openTTS/scripts/asset_download_manager.gd` — R2 HTTPRequest + sha256 streaming (web-capable)
- `/home/user/openTTS/scripts/los_rules.gd` + `terrain_overlay.gd` — line-of-sight, exists today (0.6)
- `/home/user/openTTS/scripts/undo_manager.gd` — MP-rebroadcasting undo (0.4)
- `/home/user/openTTS/scripts/save_manager.gd` — `.nml` + `serialize_game_state()` (save/resume + full-state sync)
- `/home/user/openTTS/assets/model_manifest.json` — 113 models, content-addressed, `base_url https://<legacy-cdn-host>/`
- `/home/user/openTTS/tools/model_forge/design_languages/` — 39 IP-safe faction profiles (1.0 completeness, beyond-1.0 flywheel)
- `/home/user/openTTS/docs/PRE_RELEASE_LICENSING.md` — IP/legal gate (1.0 blocker)
- `/home/user/openTTS/docs/WEB_EXPORT.md` — renderer caveats + the likely-wrong "MP won't work in-browser" line (re-test)
- `/home/user/openTTS/project.godot` — Forward+/SSAO desktop, `gl_compatibility` web
