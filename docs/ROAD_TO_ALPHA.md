...
- ✅ **Asset-CDN domain** `assets.niemandsland.xyz` **live + verified** (2026-06-18) — R2 custom domain
  on the **same bucket** as `assets.akesberg.de` (identical etag/size; no file migration). ✅ `HOST` in
  `scripts/asset_cdn.gd` switched → `https://assets.niemandsland.xyz` (_0.3.5.7_). ⚠️ **Keep
  `assets.akesberg.de` live in parallel** until every build in the wild is on 0.3.6+.
