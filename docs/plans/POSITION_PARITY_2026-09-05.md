...
The three matching runs contain 183 positions (168 games plus 15 generated cases),
...
| --- | --- | --- |
| base_shapes | 147 | PR 1 (proposed) |
| whole_unit_shorten | 70 | PR 2 (proposed) |
| boxed_escape | 16 | PR 3 (proposed) |
| accepted non-equal endpoints (difference, not decline) | 14 | PR 4 (investigate first) |
...
| Base geometry | `geom::pair_gap_m` and `State::base_shape` feed charge aiming | `mv/gate::Disc` makes oval obstacles and corrections circular |
| Large bases | Radii feed flow, clearance, terrain sampling, formation | Shape-aware final corrections and rest legality |
...
| Numeric conversions | Core explicitly models several Vector3 float32 roundings | 14 accepted non-equal positions; investigate residual arithmetic order/rounding |
