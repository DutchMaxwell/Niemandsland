...
> **Historical / superseded snapshot** — kept for the reasoning. Its measurements are a point-in-time view, not current state; see [`../../PROJECT_STATUS.md`](../../PROJECT_STATUS.md) for the live picture. Two figures below have moved a long way: the **rules epoch is 11** on `main` (#898), not 3, and every record header has stamped `rules_epoch` since #844 — see `CHANGELOG.md`. And the **`>= 55%` promotion/SHIP bar in sections 1 and 6 is retired**; the current rule is stated inline at each of those steps.
...
1. Import the promotion receipt: previous brain hash, fresh paired seeds, both seats, wins/losses/draw policy, lab commit, corpus-generation digest, **and the core sha plus rules epoch the result was measured on**. **Bar, updated 2026-09-13** — the `n >= 600` / `>= 55%` / naive sign-test wording this step used to carry is retired. The `55%` had no derivation anywhere in the record, and a bar on the point estimate gets *harder*, not easier, as `n` grows. The rule now has two numbers: a result counts as **measured** when the block-mean 95 % confidence interval's lower bound is above 50, at whatever `n` was run; a brain counts as **promoted** at **`>= 52.0%` with `n = 5000`** games on a 4-game-per-block bank, **and** not worse than the hand evaluator at 32/3. Score it with the block-mean re-scorer (`tools/arena_rescore.py`), not a sign test over naive `n`; a run scored short of its plan is refused or stamped `PARTIAL n/m`.
...
6. Run the pre-registered real-table SHIP gate against the currently shipped Classic opponent: fresh paired Godot games with seats swapped, scored under the **same two-number rule as step 1** — the block-mean 95 % CI lower bound above 50 to call the result *measured*, and **`>= 52.0%` at `n = 5000`** on a 4-game-per-block bank to ship it. Score with `tools/arena_rescore.py`, which refuses or stamps `PARTIAL n/m` a run scored short of its plan; the old `600` games / `>= 55%` / sign-test wording is retired for the reason given in step 1. Archive result, seeds, logs, game/core commits, **core sha and rules epoch**, manifest and brain hashes.
