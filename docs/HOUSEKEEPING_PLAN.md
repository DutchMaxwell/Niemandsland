...
4. **OPR data** — must never be bundled or MIT-licensed. It is already correct at
   ...
   **git history** (P3).
5. **`assets/model_manifest.json` (R2, `https://<legacy-cdn-host>/`)** — the
...
```bash
rm -rf /home/user/openTTS/.godot
# rebuild cache WITHOUT an editor save (never use --editor --quit here):
godot --headless --import --path /home/user/openTTS
grep -n 'rendering_method.web' /home/user/openTTS/project.godot  # must still print line 91
```
...
```bash
rm -rf /home/user/openTTS/dist /home/user/openTTS/build
rm -rf /home/user/openTTS/tools/model_forge/venv \
       /home/user/openTTS/tools/model_forge/_tools
git -C /home/user/openTTS clean -xnd tools/model_forge/engine_comparison/   # preview
git -C /home/user/openTTS clean -xfd tools/model_forge/engine_comparison/    # execute
rm -rf /home/user/openTTS/renders /home/user/openTTS/reports
find /home/user/openTTS -name __pycache__ -type d -not -path '*/.godot/*' -prune -exec rm -rf {} +
```
...
```bash
rm -f /home/user/openTTS/tools/model_forge/state/_ah_*.log
```
...
```bash
rm -f /home/user/openTTS/test_nodename_probe.gd.uid \
      /home/user/openTTS/dao_union_gallery*.png \
      /home/user/openTTS/dao_union_gallery*.png.import \
      /home/user/openTTS/assets/startup_background.png.import
```
...
```bash
rm -rf /home/user/openTTS/tools/model_forge/state/_archived_* \
       /home/user/openTTS/tools/model_forge/state/_discarded_* \
       /home/user/openTTS/tools/model_forge/state/_samurai_test \
       /home/user/openTTS/tools/model_forge/state/_overnight_done_*
```
...
printf '\n# Terrain GLBs + companion images are R2-delivered, never commit them\nassets/terrain/grimdark_industrial/\n' \
  >> /home/user/openTTS/.gitignore
git -C /home/user/openTTS check-ignore assets/terrain/grimdark_industrial/trees/mutated_pine.glb  # must now print the path
```
...
```bash
git -C /home/user/openTTS gc
git -C /home/user/openTTS count-objects -vH
```
...
```bash
git -C /home/user/openTTS grep -n 'SESSION_HANDOFF' -- '*.md'  # find back-refs first
git -C /home/user/openTTS rm docs/SESSION_HANDOFF.md
...
```bash
mkdir -p /home/user/openTTS/docs/archive
git -C /home/user/openTTS mv docs/AAA_UI_PLAYBOOK.md docs/archive/AAA_UI_PLAYBOOK.md
git -C /home/user/openTTS mv docs/PLAN_UNIT_SYSTEM.md docs/archive/PLAN_UNIT_SYSTEM.md
```
...
```bash
git -C /home/user/openTTS rm docs/UI_MODERNIZATION_PLAN.md  # or: git mv ... docs/archive/
```
...
Reality (live manifest + CHANGELOG): **Cloudflare R2, `<legacy-cdn-host>`,
...
```bash
python3 -c "import json;print(json.load(open('/home/user/openTTS/assets/model_manifest.json'))['base_url'])"  # source of truth
grep -n 'Planned' /home/user/openTTS/docs/ARCHITECTURE.md
```
...
```bash
git -C /home/user/openTTS rm -r examples/         # P-DEC-3
git -C /home/user/openTTS rm -r tools/3d-generation  # P-DEC-4
```
...
- **Fix `export_presets.cfg`:** macOS `bundle_identifier="niemandsland"` is
  invalid (needs reverse-DNS, e.g. `de.<redacted>.niemandsland`); add
...
```bash
git -C /home/user/openTTS checkout main
git -C /home/user/openTTS branch -D backup/cloud-3d3441b
```
