...
fn the_gate_clears_a_recorded_base_overlap_inside_every_band_cap() {
    ...
    let (got, rep) = finalize_placement(&c.planned, &c.radii_in, &[], &c.caps_in, c.board_in);
...
fn pass_one_clamps_a_model_back_onto_the_table() {
    ...
    let (got, rep) = finalize_placement(&planned, &[0.5, 0.5], &[], &[], board);
    assert!(
...
fn the_gate_pushes_off_another_units_base() {
    ...
    let (got, _) = finalize_placement(&planned, &[0.8], &[other], &[4.0], [72.0, 48.0]);
    let d =
