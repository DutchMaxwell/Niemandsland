...
/// The ANTI-CHAIN clause, verbatim: "May not be used if this unit was activated
/// via Coordinate." A bearer that itself took a hand-off this round hands
/// nothing on, so at most TWO activations ever ride one hand-off.
#[test]
fn a_bearer_that_took_a_hand_off_may_not_pass_one_on() {
    let (mut st, statics) = hand_off_line(CURRENT_RULES_EPOCH);
    let a = idx(&st, "p1_0_a");
    st.coordinate_via_round[a] = st.round;
    let end = truncated(&st, &statics, CURRENT_RULES_EPOCH, 1);
    assert!(
        !end.activated[idx(&st, "p1_1_b")],
        "the bearer was activated via Coordinate, so it may not use it"
    );
}

/// The stamp is ROUND-scoped, not permanent: the table erases its bool at the
/// round reset ("a unit coordinated in round 2 may hand off again in round 3"),
/// and a stamp from an EARLIER round must not gag the bearer today.
#[test]
fn a_stamp_from_an_earlier_round_does_not_gag_the_bearer() {
    let (mut st, statics) = hand_off_line(CURRENT_RULES_EPOCH);
    let a = idx(&st, "p1_0_a");
    st.coordinate_via_round[a] = st.round - 1;
    let end = truncated(&st, &statics, CURRENT_RULES_EPOCH, 1);
    assert!(
        end.activated[idx(&st, "p1_1_b")],
        "last round's hand-off is spent; this round's is the bearer's to give"
    );
}

/// The table bridge. `AiActRecorder._ledger_of` exports the receiver's
/// `unit_properties["activated_via_coordinate"]`, and a replayed act must read
/// it back: a chain the TABLE already refused cannot come back to life on
/// replay (the #493/#498 divergence shape — a once-per-round flag silently
/// vanishing between activations).
#[test]
fn the_ledger_restores_an_anti_chain_stamp_set_on_the_table() {
    let plain = PLAIN.replace(
        r#""p1_0_a":{"player":1,"#,
        r#""p1_0_a":{"ledger":{"activated_via_coordinate":true},"player":1,"#,
    );
    let header = read_act_header(HEADER).expect("header");
    let mut cache = ProfileCache::new(header.profiles);
    let mut roster = None;
    let st = io::state_from_json(&plain, &mut cache, &mut roster).expect("state");
    let a = idx(&st, "p1_0_a");
    assert_eq!(st.coordinate_via_round[a], st.round, "the table's stamp survives the fold");
    let statics = statics_of(&st, CURRENT_RULES_EPOCH);
    let end = truncated(&st, &statics, CURRENT_RULES_EPOCH, 1);
    assert!(
        !end.activated[idx(&st, "p1_1_b")],
        "the chain was refused on the table, and it stays refused here"
    );
}

/// The registry is the reach's source, not a literal in this file: the shipped
/// `Coordinate` entry carries `range_in: 12.0` in both systems, and a unit that
/// does not carry the rule gets `0.0` — "not a carrier", the field's own null.
#[test]
fn the_reach_comes_off_the_registry_entry() {
    let (st, statics) = hand_off_line(CURRENT_RULES_EPOCH);
    let carrier = &statics[st.roster.profile[idx(&st, "p1_0_a")]];
    let plain = &statics[st.roster.profile[idx(&st, "p1_1_b")]];
    assert_eq!(carrier.coordinate_range_in, 12.0, "the shipped gf entry's own range_in");
    assert_eq!(plain.coordinate_range_in, 0.0, "no rule, no reach");
}
