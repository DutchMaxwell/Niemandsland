use super::*;
    ...
    fn fatigue_is_a_flat_six_that_no_bonus_reaches() {
        ...
        assert_eq!(melee_hit_target(&p, &att, &defender(4, 1), true, 0, 0.0, false).0, 2,
                   "Reliable 2+, and Thrust cannot go below the 2+ floor");
        let tired = Ctx { fatigued: true, ..att };
        assert_eq!(melee_hit_target(&p, &tired, &defender(4, 1), true, 0, 0.0, false).0, 6);
        assert_eq!(melee_hit_target(&p, &tired, &defender(4, 1), true, 1, 0.0, false).0, 6,
    ...
    fn unstoppable_clamps_the_summed_modifier_once() {
        ...
        assert_eq!(melee_hit_target(&p, &att, &evasive, false, 1, 0.0, false).0, 6,
        ...
        assert_eq!(melee_hit_target(&blade(1), &plain, &evasive, false, 0, 0.0, false).0, 6,
