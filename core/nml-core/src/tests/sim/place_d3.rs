use super::*;
    ...
    // ---- TEST WAVE (2026-09-15, D-PROOF part 2 chunk 4 of 4) — "Wave-Step"'s
    // own NUMBER pins (aof/deep_sea_elves, the `Bounding {place_d3_plus: 0}`
    // DATA-alias the ledger's sweep C exercised only inside the Wave-Step
    // Boost's test): the same shape as the Rapid Blink pins above — the
    // placement read stamps the alias's own PlaceSpec (ONE die, plus 0.0),
    // the activation hops the plain 6" band by the rolled D3 and names the
    // rule, and a below-gate epoch stays on the plain band.

    /// The carrier: an aof/deep_sea_elves profile whose ONLY rule is
    /// "Wave-Step", read off the REAL registry. The REAL `build_for`
    /// product, at `epoch`.
    fn wave_step(epoch: u32) -> UnitStatic {
        let p = Profile {
            ...
            model_count: 1,
            weapons: vec![],
            special_rules: vec!["Wave-Step".into()],
            caster_value: 0,
            base_radius: 0.0,
            base_shape: String::new(),
            base_w_mm: 0.0,
            ...
            faction_folder: "deep_sea_elves".into(),
            item_grants: vec![],
            attached_hero_rules: vec![],
            move_bands: MoveBands { advance: 6.0, rush: 12.0, charge: None },
        };
    ...
    /// The hop line with the Wave-Step carrier in slot 0 (the Rapid Blink
    /// corridor: the other slots leave the board so the scan has a clear
    /// corridor toward the objective).
    fn wave_line(epoch: u32) -> (State, Vec<UnitStatic>) {
        let (mut st, _) = dangerous_line();
        for j in 1..4 {
            st.positions[j] = vec![];
            st.radii[j] = vec![];
        ...
        (st, vec![wave_step(epoch), UnitStatic { name: "ah".into(), ..Default::default() },
            UnitStatic { name: "b".into(), ..Default::default() },
            UnitStatic { name: "bh".into(), ..Default::default() }])
    }

    /// The placement read's own numbers: one die, plus 0.0 (the alias's own
    /// `place_d3_plus: 0`), under the rule's EXACT name.
    #[test]
    fn wave_step_stamps_its_own_place_spec_at_the_current_epoch() {
        let (_, statics) = wave_line(crate::acts::CURRENT_RULES_EPOCH);
        let spec = statics[0].bounding_place.as_ref().expect("the alias stamps its placement read");
        assert_eq!(spec.name, "Wave-Step", "the rule name the table logs");
        assert_eq!(spec.dice, 1, "one D3");
        assert_eq!(spec.plus, 0.0, "the alias's own place_d3_plus: 0");
    }

    /// At the CURRENT epoch a fresh-sim Wave-Step activation hops the unit
    /// toward the objective BEFORE the move — the ADVANCE lands past the
    /// plain 6" band, and the hop names the rule (rules-must-log). The roll
    /// is 1..=3, the scan takes a strictly closer legal spot only, so
    /// 6.5"-9.5" covers every face.
    #[test]
    fn at_the_current_epoch_a_wave_step_activation_hops_before_the_move() {
        let (st, statics) = wave_line(crate::acts::CURRENT_RULES_EPOCH);
        let (next, shot) = run_hop(&st, &statics, crate::acts::CURRENT_RULES_EPOCH);
        let moved_in = (next.positions[0][0][0] - st.positions[0][0][0]) / IN2M;
        assert!(
            moved_in > 6.5 && moved_in < 9.5,
        ...
        assert!(
            shot.log.iter().any(|l| l.contains("Wave-Step") && l.contains("rolled")),
            "rules-must-log: one trace line naming the rule and the rolled distance, got {:?}",
            shot.log
        );
    }

    /// Below the gate (EPOCH_25_ETHEREAL_BANDS) a fresh-sim Wave-Step
    /// activation does NOT hop — the ADVANCE is the plain 6" band to the
    /// digit, and the placement read is `None`.
    #[test]
    fn at_epoch_25_a_wave_step_activation_stays_on_its_plain_band() {
        let (st, statics) = wave_line(crate::acts::EPOCH_25_ETHEREAL_BANDS);
        assert!(statics[0].bounding_place.is_none(), "below the gate the placement read is None");
        let (next, shot) = run_hop(&st, &statics, crate::acts::EPOCH_25_ETHEREAL_BANDS);
        let moved_in = (next.positions[0][0][0] - st.positions[0][0][0]) / IN2M;
        assert!((moved_in - 6.0).abs() < 1e-6, "plain band only: {moved_in}\"");
