...
/// v206 — trip-survival discount on `presence`: `reply_threat` is ONE activation
/// of every living enemy, so a unit `trip` rush-moves from the marker expects
/// `threat * trip` reply wounds before it arrives; the surviving share of its
/// strength fades the hold claim. At the marker (trip 0) this is bit-for-bit v0.
fn presence_v206(state: &State, i: usize, obj_pos: [f64; 3], threat: f64) -> f64 {
    let base = presence(state, i, obj_pos, threat);
    if base <= 0.0 || threat <= 0.0 {
        return base;
    }
    let strength: f64 = state.wounds[i].iter().map(|&w| w as f64).sum();
    let trip = (control_gap_in(state, i, obj_pos) - OBJECTIVE_CONTROL_IN).max(0.0)
        / state.bands[i].rush.max(1.0);
    base * ((strength - threat * trip) / strength).clamp(0.0, 1.0)
}

/// v206 hand: v0's marker accumulation with `presence_v206` as the hold
/// strength; destroy missions and no-unit markers keep the frozen answers.
fn score_hand_v206(state: &State, player: i64, incoming: Incoming) -> f64 {
    ...
    for i in 0..state.objectives.len() {
        ...
        for u in 0..state.units() {
            let p = presence_v206(state, u, obj.pos, threat_of(incoming, u));
            if state.player[u] == player { mine += p } else { theirs += p }
        }
        total += if mine + theirs > 0.0 {
            mine / (mine + theirs)
        } else if obj.owner == 0 { 0.5 } else if obj.owner == player { 1.0 } else { 0.0 };
    }
    total / state.objectives.len() as f64
}
...
pub fn score_hand_variant(state: &State, player: i64, incoming: Incoming, eval_variant: i64) -> f64 {
    match eval_variant {
        ...
        206 => score_hand_v206(state, player, incoming),
