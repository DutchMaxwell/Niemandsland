...
/// v401 trip survival: reply wounds bite on every marching round too, so of a
/// `w`-wound unit only `(1 - t/w)^needed` arrives (v0 books full strength).
fn survival_v401(state: &State, i: usize, obj_pos: [f64; 3], threat: f64) -> f64 {
    let w: f64 = state.wounds[i].iter().map(|&x| x as f64).sum();
    if threat <= 0.0 || w <= 0.0 {
        return 1.0;
    }
    let d = control_gap_in(state, i, obj_pos);
    if !d.is_finite() { return 1.0; } // nothing on the table: presence is 0 anyway
    let mut needed = (((d - OBJECTIVE_CONTROL_IN) / state.bands[i].rush.max(1.0)).ceil() as i64).max(0);
    if state.shaken[i] { needed += 1; }
    (1.0 - threat / w).max(0.0).powf(needed as f64)
}

/// v401 mirror of `score_hand`: destroy lane stays frozen v0; marker-control
/// lane discounts each unit by trip survival.
fn score_hand_v401(state: &State, player: i64, incoming: Incoming) -> f64 {
    if state.objectives.is_empty() {
        return 0.5;
    }
    if !state.markers_meta.is_empty() && is_destroy_mission(state) {
    ...
    for obj in &state.objectives {
        ...
        let mut theirs = 0.0f64;
        for i in 0..state.units() {
            let t = threat_of(incoming, i);
            let p = presence(state, i, obj.pos, t) * survival_v401(state, i, obj.pos, t);
...
pub fn score_hand_variant(state: &State, player: i64, incoming: Incoming, eval_variant: i64) -> f64 {
    match eval_variant {
        ...
        401 => score_hand_v401(state, player, incoming),
