...
use crate::unit::{Ctx, UnitStatic, ShootProfile, UtilityBuff, StormFacet};
#[cfg(test)]
...
/// through the same seam, shared too with unit.rs's build-time waves (the
/// wave-3 aura expansion read logs its fired grants through the same
/// NML_TRACE_RULES seam).
