...
pub struct UnitStatic {
    ...
    /// 40 the unit rolls the recovery die instead (`battleborn_recovery_roll`
    /// off `battleborn_recover_target`'s Steadfast alias arm, the same seeded
...
///
/// Sweep H row Steadfast: from the FROZEN `EPOCH_40_STEADFAST_ROLL` the
/// plain "Steadfast" entry joins the die-roll recover aliases — the table's
/// own picker (`_solo_round_start_recovery_rule`'s second arm, main.gd) is
...
pub struct UtilityBuff {
    ...
    /// `range_bonus_in` (documented NO, design §4(d) step 3). The READS are
    /// PR 2 — nothing folds these yet.
    pub ap_mod: i64,
    pub def_mod: i64,
    pub defense_mod: i64,
    /// `move_mod` — the family's move-only knob (Great Musician, aof). Parsed
    /// ONLY from `EPOCH_12_MOVE_BUFF` on; below 12 it stays 0 and the all-zero
    /// row keeps being dropped.
    pub move_mod: i64,
...
/// `Role::Casting` is never summed, and `range_bonus_in` is not
/// modeled on `UtilityBuff` (`move_mod` joined the record shape AND its
/// move-seam read at `EPOCH_12_MOVE_BUFF`, the Great Musician port) —
/// `record_buff` drops the all-zero row (`ap_mod`/
...
fn fatigue_debuff_of(reg: &mut Registries, p: &Profile, rules_epoch: u32) -> Vec<FatigueDebuffSpec> {
    if !rule_on(rules_epoch, EPOCH_7_TABLE_RULES) {
        return Vec::new();
    }
    let map = reg.rules_for(&p.game_system);
...
fn utility_buffs_of(reg: &mut Registries, p: &Profile, rules_epoch: u32, un: &mut Vec<Unimplemented>) -> Vec<UtilityBuff> {
    ...
    for raw in raws {
        ...
        out.push(UtilityBuff {
            ...
            // The MOVE-BUFF knob (the Great Musician port, the FROZEN
            // `EPOCH_12_MOVE_BUFF`): from epoch 12 a row whose ONLY knob is
            // `move_mod` lands on `record_buff`'s ledger AND rides the move
        ...
        if !b.vs_target && b.reposition_in <= 0.0 && b.grants_rule.is_empty()
            ...
            un.push(Unimplemented { rule: b.name.clone(), why:
                "Utility Buff params carry no hit/casting/morale/ap/def/move mod and no grants_rule, so this resolver records nothing — main.gd:16534 builds the same three keys and _solo_record_spell_mod:3663 drops the all-zero row. If the rule works on the table it does so at a seam this port does not claim".into() });
        }
    }
    out
}
...
fn move_rule_mods_of(reg: &mut Registries, p: &Profile, rules_epoch: u32) -> Option<Bands> {
    ...
    // `advance_mod`/`rush_mod` (-2/-4) onto BOTH bands, the loader twin's
    // band fold (list_to_profile.py:537-543, the table's name fallback
    // movement_range_controller.gd:116-121; the registry's `common` "Slow"
    // entries carry exactly those params, gf+aof 3.5.1). Evidence-only
    // standing like the rest of this fold (the accepted `bounding` shape,
...
impl UnitStatic {
    ...
    let (hnr_move_in, hnr_rule) = hit_and_run_boost_of(reg, p, rules_epoch);
        UnitStatic {
            ...
            // Sweep H row Steadfast — the free-clear gate flag: the round-start
            // Shaken clear for Steadfast stays free only BELOW the FROZEN
            // `EPOCH_40_STEADFAST_ROLL`; from 40 the playout rolls the die
            // instead (rollout.rs `battleborn_recovery_roll` off the alias
