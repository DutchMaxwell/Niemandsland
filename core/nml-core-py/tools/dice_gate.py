...
def run(ref: Path, repo: str, limit: int, out: str, red: str, report_only: bool,
    ...
    print()
    print("GATE D1-B6 over %d games, %d activations, movement=%s%s, %s%s%s (%.1fs)"
          % (len(games), acts, movement,
             " — RED --red-move-rigid (ADVANCE/RUSH back on the rigid arm)"
             if move_rigid else "",
             vintage_report_line(vintage_seen),
...
def main(argv: list[str]) -> int:
    ...
    ap.add_argument("--red-move-rigid", action="store_true",
                    help="NML-1152 S3 RED: header knob move_rigid — ADVANCE and RUSH stay on "
                         "the rigid translation while --movement table still routes CHARGE. "
                         "THIS gate replays only acts that DRAW dice (a shot, a charge), so a "
