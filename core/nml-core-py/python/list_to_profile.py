...
# ------------------------------------------------------ Split step 2: templates ---
# D-SPLIT (decided 2026-09-15, option (b) of analysis/SPLIT_STEP2_2026-09-14.md):
# a FRESH core sim refuses a fielded `Split(<name> [<n>])` carrier at load until
# the header carries the child's template (io.rs:1111 `spawn_templates_of` ->
# "no spawn_profiles template for unit key ..."), so the loader ships the child
# templates under `spawn:<carrier_key>:<rule string>` — the SAME map #949 gave
# recorded games (act_recorder.gd `_spawn_profiles`), which the fresh path
# already indexes (acts.rs:header_of -> io.rs:index_spawn_profiles). The child's
# profile is resolved by the table's NAMED BOOK lookup (main.gd:18135-18138 ->
# opr_api_client.gd:1792-1800, `_parse_named_unit`: scan the book's `units` for
# the string's name, build a FULL fresh unit, stamp size = count) — never off
# the parent: parent-side derivation would be the #823 silent-fallback break.
# The book comes off the table's own snapshot instrument (opr_api_client.gd:50-61):
# `<NML_ARMY_BOOKS_DIR>/<system>/<armyId>.json`; a book that never landed drops
# the child LOUDLY (one warning), never a substitute profile.

#: scripts/opr_api_client.gd:61 (SNAPSHOT_DIR_ENV) — the private army-book
#: snapshot directory the named lookup reads. The Godot `user://army_books/`
#: default is Godot-FS-only; the trainer's instrument is the env.
ARMY_BOOKS_DIR_ENV = "NML_ARMY_BOOKS_DIR"

#: scripts/main.gd:18136 — the SAME unit-ref regex the beat and the recorder
#: compile (rules.rs:spawn_rule_target reads the same shape):
#: `Split(Goblin Mob [4])` -> ("Goblin Mob", 4).
_RULE_UNIT_REF_RE = re.compile(r"^[^(]+\((.+) \[(\d+)\]\)$")

_BOOKS_INDEX: dict[str, Any] | None = None
_BOOK_CACHE: dict[tuple[str, str], dict[str, Any]] = {}


def _army_books_index() -> dict[str, Any]:
    """opr_api_client.gd:_books_index — the PUBLIC text-free index
    (assets/solo/army_books_index.json, generated by tools/army_books_index.py;
    no rule text ever enters this repo, THIRD_PARTY.md): armyId -> book name
    + faction slug, per game system. Unreadable -> {}, the caller's ladder
    answers with no book."""
    global _BOOKS_INDEX
    if _BOOKS_INDEX is None:
        try:
            _BOOKS_INDEX = json.loads(
                (REGISTRY_DIR / "army_books_index.json").read_text(encoding="utf-8")
            )
        except (OSError, ValueError):
            _BOOKS_INDEX = {}
    return _BOOKS_INDEX
...
def _army_book(data: dict[str, Any], system: str, faction: str) -> dict[str, Any]:
    """opr_api_client.gd:_cached_book (:1774-1789) MINUS the network rung:
    the list's own armyId first (all units share it, :608), else the index's
    faction match, then the snapshot file `<dir>/<system>/<armyId>.json`.
    {} on any miss — the caller drops the child loudly, never substitutes."""
    book_id = ""
    units = data.get("units", []) or []
    if units and isinstance(units[0], dict):
        book_id = str(units[0].get("armyId", "") or "")
    if not book_id:
        for candidate, entry in _army_books_index().get(system, {}).items():
            if str(entry.get("faction_folder", "")) == faction:
                book_id = candidate
                break
    if not book_id:
        return {}
    ...
    if key not in _BOOK_CACHE:
        book: dict[str, Any] = {}
        base = os.environ.get(ARMY_BOOKS_DIR_ENV, "")
        if base:
            try:
                loaded = json.loads(
                    (Path(base) / system / (book_id + ".json")).read_text(encoding="utf-8")
                )
            except (OSError, ValueError):
                loaded = None
            # _snapshot_book (:162-170): a payload without its name is no book.
...
def _split_rule_refs(special_rules: list[str]) -> list[tuple[str, str, int]]:
    """The parametrised `Split(<name> [<n>])` strings of one profile, in order:
    `(trimmed raw string, unit_name, count)`. The core's own read
    (rules.rs:spawn_rule_target over the same `rule_name_matches`): a bare
    name, a malformed bracket or a non-positive count is no target and never
    demands a template."""
    out: list[tuple[str, str, int]] = []
    for raw in special_rules:
        if _base_rule(str(raw)) != "Split":
            continue
        m = _RULE_UNIT_REF_RE.match(str(raw).strip())
        if m is None:
            continue
        name = m.group(1).strip()
        count = int(m.group(2))
        if name and count > 0:
            out.append((str(raw).strip(), name, count))
    return out


def _book_definition(book: dict[str, Any], unit_name: str) -> dict[str, Any]:
    """opr_api_client.gd:_parse_named_unit (:1792-1800): the book's `units`
    scanned for the string's NAME; the FIRST match answers, {} otherwise."""
    for definition in book.get("units", []) or []:
        if str(definition.get("name", "")) == unit_name:
            return definition
    return {}


def _book_special_rules(definition: dict[str, Any]) -> list[str]:
    """opr_api_client.gd:_parse_unit_from_list (:1512-1523): the definition's
    own specialRules — plain strings verbatim, dicts through _rule_to_string
    (the type-safe formatter, NML-1073 M3-3b)."""
    rules: list[str] = []
    for r in definition.get("specialRules", []) or []:
        if isinstance(r, str):
            if r:
                rules.append(r)
        else:
            rl = _rule_to_string(r)
            if rl:
                rules.append(rl)
    return rules


def _book_weapons(definition: dict[str, Any]) -> list[dict[str, Any]]:
    """opr_api_client.gd:_parse_weapon (:1735-1752) over the definition's
    `equipment` + `loadout` -> the loader's internal weapon dicts
    (name/range/attacks/count/rules), the shape `_unit_profile` reads."""
    out: list[dict[str, Any]] = []
    for w in (definition.get("equipment", []) or []) + (definition.get("loadout", []) or []):
        if isinstance(w, str):
            out.append({"name": w, "range": 0, "attacks": 1, "count": 1, "rules": []})
            continue
        if not isinstance(w, dict):
            continue
        wrules: list[str] = []
        for r in w.get("specialRules", []) or []:
            if isinstance(r, str):
                if r:
                    wrules.append(r)
            else:
        ...
        out.append(
            {
                "name": str(w.get("name", w.get("label", "Unknown Weapon"))).strip(),
                "range": int(w.get("range", 0)),
                "attacks": int(w.get("attacks", 1)),
...
def _split_child_template(
    ...
    """One child template: main.gd:_solo_create_rule_unit ->
    opr_api_client.gd:_parse_unit_from_list over the BOOK definition, with
    `size = count` stamped (:1797) — the child's OWN quality/defense/rules/
    weapons/base, `wounds_max` of `count` entries of its own per-model Tough —
    then the loader's `_unit_profile`, the SAME builder the profiles map
    rides, with `unit_id` = the spawn key."""
    rules = _book_special_rules(definition)
    tough = max(_unit_rating(rules, "Tough"), 1)
    u: dict[str, Any] = {
        "unit_id": key,
        "name": str(definition.get("name", "Unknown Unit")),
        "quality": int(definition.get("quality", 4)),
        "defense": int(definition.get("defense", 4)),
        "special_rules": rules,
        "item_grants": {},
        "weapons": _book_weapons(definition),
        "model_tough": [tough] * count,
        # The child's own base recommendation, the Tough fallback behind it —
        # the table reads the child's base off ITS OWN profile too
        # (main.gd:18145-18149).
        "base": _base_of(definition, game_system, rules),
    }
    return _unit_profile(u, faction, game_system)


def spawn_profiles_from_army_forge_json(
    data: dict[str, Any], faction: str, player: int
) -> dict[str, dict[str, Any]]:
    """SPLIT step 2 — the header's `spawn_profiles` value for a FRESH sim:
    one child template per parametrised `Split(<name> [<n>])` string a fielded
    unit carries, keyed `spawn:<carrier_key>:<rule string>` the way #949's
    replay reader expects. The `profiles` map itself is NOT touched —
    `profiles_from_army_forge_json` keeps the field-for-field M3-3 contract;
    the templates ride the separate map, like the recorder's header write
    (indexed on the fresh path too by acts.rs:header_of ->
    io.rs:index_spawn_profiles). The book is read at most once per list, and
    only when a Split string exists at all — a list without one never touches
    the snapshot. The carrier build rides the SAME two passes the profiles map
    runs (manifest base overrides, aura expansion), so the carrier keys and
    rule strings match `profiles_from_army_forge_json`'s exactly."""
    game_system = str(data.get("gameSystem", "gf"))
    built = _units_from_list(data, player)
    _apply_manifest_base_overrides(built, faction)
    _expand_auras(built)
    ...
    out: dict[str, dict[str, Any]] = {}
    for u in built:
        for raw, unit_name, count in _split_rule_refs(u["special_rules"]):
            key = "spawn:%s:%s" % (u["unit_id"], raw)
            if book is None:
                book = _army_book(data, game_system, faction)
            definition = _book_definition(book, unit_name)
            if not definition:
                warnings.warn(
                    "list_to_profile: split child '%s' of %s has no army-book "
                    "snapshot definition (NML_ARMY_BOOKS_DIR=%r) — template "
                    "dropped, never substituted" % (
                        unit_name, key, os.environ.get(ARMY_BOOKS_DIR_ENV, "")
                    )
                )
                continue
            out[key] = _split_child_template(definition, key, count, faction, game_system)
            print(
                "list_to_profile: split child template %s shipped (%s x%d)"
                % (key, unit_name, count)
            )
    return out


def spawn_profiles_from_list(path: str | Path, player: int) -> dict[str, dict[str, Any]]:
    """The list-FILE entry point, `profiles_from_list`'s twin: an Army-Forge
    list JSON + its player seat -> the header's `spawn_profiles` map ({} when
    the list fields no Split carrier — no key, byte-identical headers)."""
    with open(path, encoding="utf-8") as f:
        data = json.load(f)
    return spawn_profiles_from_army_forge_json(
        data, _faction_from_path(path, str(data.get("gameSystem", ""))), player
    )
