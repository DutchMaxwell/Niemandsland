...
use nml_core::state::{MoveBands, Roster};
use nml_core::{Marker, Mods, Objective, Profile, Profiles, State, Weapon};
...
pub fn profile_of(d: &VarDictionary) -> Profile {
    ...
    Profile {
        ...
        attached_hero_rules: darr(d, "attached_hero_rules")
            .iter_shared()
            .map(|v| v.try_to::<VarArray>().map(|a| strings(&a)).unwrap_or_default())
            .collect(),
...
pub fn build_state(
    ...
    let markers_meta: Vec<Marker> = darr(plain, "markers_meta")
        ...
        .collect();
    let los_pairs = plain.get("los_pairs").and_then(|v| v.try_to::<VarArray>().ok()).map(|rows| {
