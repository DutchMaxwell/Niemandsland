...
pin = json.loads((p.FIXTURES.parent / 'base_shapes.json').read_text())
...
with tempfile.TemporaryDirectory(prefix='position-parity-seam-') as directory:
    ...
    assert 'base_shapes' in row['table_stages']
    assert 'base_shapes' in row['rust_capabilities']
...
"#])
    ...
    assert!(result.success(), "the pinned shape fixture must cross the live MOVE seam");
}
