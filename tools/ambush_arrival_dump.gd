...
## booted board/table size. CASES below is data, not per-case code, to keep this tool's line
## count out of the ~60-line ARRIVAL gate budget (SPEC §4 S4) while still covering the ring
## arithmetic: plain Ambush (9"), Infiltrate (3"), Repel Ambushers' 12" override, mixed/multi
## enemies, a pre-occupied seed, wide/small/large footprints, a Strider flag, and two HELD
## (no-legal-spot) cases.
##
...
## [case name, rules (comma-joined), arriver model count, arriver base mm,
##  enemies [[rule or "", Vector3 pos], ...], objective Vector2, occupied [[Vector2 pos, radius], ...]]
const CASES := [
	["ambush_single_enemy_centre", "Ambush", 1, 32, [["", Vector3(0.05, 0, 0.05)]], Vector2(0.3, 0.2), []],
	["infiltrate_single_enemy_centre", "Infiltrate", 1, 32, [["", Vector3(0.05, 0, -0.05)]], Vector2(-0.2, 0.1), []],
	["infiltrate_vs_repel_ambushers", "Infiltrate", 1, 32, [["Repel Ambushers", Vector3.ZERO]], Vector2(0.4, -0.3), []],
	["ambush_vs_repel_ambushers", "Ambush", 1, 32, [["Repel Ambushers", Vector3(0.1, 0, 0.1)]], Vector2(-0.4, -0.2), []],
	["ambush_two_enemies_mixed", "Ambush", 1, 32,
		[["", Vector3(0.2, 0, 0.1)], ["Repel Ambushers", Vector3(-0.5, 0, -0.3)]], Vector2(0.0, 0.4), []],
	["infiltrate_multi_enemy_no_repel", "Infiltrate", 1, 32,
		[["", Vector3(0.15, 0, -0.1)], ["", Vector3(-0.2, 0, 0.15)]], Vector2(0.5, 0.5), []],
	["ambush_pre_occupied", "Ambush", 1, 32, [["", Vector3.ZERO]], Vector2(0.5, 0.3), [[Vector2(0.5, 0.3), 0.3]]],
	["ambush_five_model_wide_footprint", "Ambush", 5, 32, [["", Vector3(0.1, 0, -0.2)]], Vector2(-0.3, 0.3), []],
	["infiltrate_single_model_small_base", "Infiltrate", 1, 25, [["", Vector3(-0.1, 0, 0.05)]], Vector2(0.2, -0.4), []],
	["ambush_large_base_unit", "Ambush", 1, 60, [["", Vector3(0.2, 0, 0.2)]], Vector2(-0.5, -0.1), []],
	["ambush_strider_flag", "Ambush,Strider", 1, 32, [["", Vector3(-0.15, 0, -0.15)]], Vector2(0.6, 0.0), []],
	["infiltrate_objective_pull", "Infiltrate", 1, 32, [["", Vector3.ZERO]], Vector2(0.8, 0.55), []],
	["held_fully_occupied", "Ambush", 1, 32, [["", Vector3(1.0, 0, 1.0)]], Vector2.ZERO, [[Vector2.ZERO, 2.0]]],
	["held_wide_unit_occupied", "Infiltrate", 4, 40, [["", Vector3(1.0, 0, 1.0)]], Vector2.ZERO, [[Vector2.ZERO, 2.0]]],
]
...
func _drive() -> void:
	...
	_main = current_scene
	if _main == null:
		printerr("AMBUSH_DUMP FATAL: main.tscn never mounted"); quit(1); return
	...
	var cases: Array = []
	for spec in CASES:
		cases.append(_run_case(spec, zone))
	...
	if f == null:
		printerr("AMBUSH_DUMP FATAL: cannot write %s" % _out_path); quit(1); return
	f.store_string(JSON.stringify(out, "  "))
	f.close()
	print("AMBUSH_ARRIVAL_DUMP %d %d OK" % [cases.size(), held])
	# Hygiene: GameUnit.models <-> ModelInstance.unit is a reference cycle plain refcounting
	# never collects — break it and free the spawned Node3D models before quit.
	for u in _units:
		for m in (u as GameUnit).models:
			var mi := m as ModelInstance
			if is_instance_valid(mi.node):
				mi.node.free()
			mi.unit = null
...
func _run_case(spec: Array, zone: Rect2) -> Dictionary:
	...
	for i in range(int(spec[2])):
		positions.append(Vector3(6.0 + float(_units.size()) * 0.15 + float(i) * 0.1, 0, 0))
	...
	arriver.unit_properties["base_size_round"] = int(spec[3])
	_units.append(arriver)
	var enemies: Array = []
	for e in spec[4] as Array:
		var enemy: GameUnit = E2EBoot.make_unit(_main, 1, "%s_e" % spec[0], [e[1]])
		if str(e[0]) != "":
			enemy.unit_properties["special_rules"] = [str(e[0])]
		_units.append(enemy)
		enemies += _enemy_entries(enemy)
	var occupied: Array = []
	for o in spec[6] as Array:
		occupied.append({"pos": o[0], "radius": o[1]})
	return _record(spec[0], arriver, zone, [spec[5]], occupied, enemies)
