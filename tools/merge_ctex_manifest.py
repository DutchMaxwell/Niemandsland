...
Usage:  python3 tools/merge_ctex_manifest.py [MANIFEST [PATCHES_DIR]]
"""
import glob
import json
import sys
...
def main(argv: list) -> int:
    manifest_path = argv[1] if len(argv) > 1 else "assets/model_manifest.json"
    patches_glob = (argv[2] if len(argv) > 2 else "ctex_patches") + "/*.json"
    with open(manifest_path) as f:
        man = json.load(f)
    models = man["models"]
    patches: dict = {}
    for path in sorted(glob.glob(patches_glob)):
        with open(path) as f:
